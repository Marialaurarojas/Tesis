...
En \texttt{scipy}, con la función \texttt{RealData(x, y, sx=sx, sy=sy)} se incluyen los valores del error de medición asociado a cada punto $x_i$ e $y_i$ como los correspondientes pesos en la ecuación de sumatoria~\ref{eq:S}:
\begin{equation}
...
Usando las consideraciones anteriores, integrando las ecuaciones~\ref{eq:diagonal}:
...
y en las ecuaciones~\ref{eq:fuera_diag}:
