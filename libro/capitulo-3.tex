...
\begin{equation}
f_0=\frac{c}{2\pi\sqrt{\dfrac{L\,A_c}{d\,\mu_0}}}=k\,c
\end{equation}
...
\textbf{Primer montaje experimental: Circuito RLC conectado a una fuente variable}

% \begin{figure}[h!]\centering\includegraphics[width=0.7\linewidth]{montaje1.png}\caption{Esquema del primer montaje.}\end{figure}

Esta primera forma está basada en la referencia [1] donde se usa el siguiente esquema. En nuestro caso se propone prescindir de la resistencia por la razón antes expuesta.
...
En esta segunda forma se conectan la bobina y el condensador en serie, luego se enrolla un cable no aislado alrededor del inductor y los finales de este a un generador de frecuencia, de igual modo se usa otro cable para enrollarlo alrededor del condensador y se conectan sus extremos a un osciloscopio. Este montaje está basado en la referencia [2]. En este caso la corriente variable $i_0(t)$ induce una fem $\varepsilon$ en el circuito (ley de Faraday) que varía con la misma frecuencia que la corriente original, a su vez $\varepsilon$ genera una corriente inducida $i(t)$ en el circuito LC que tiene la misma frecuencia que $i_0(t)$. Para realizar las mediciones, en la misma referencia [2] se plantea enrollar otro cable no aislado alrededor del inductor (en otra zona del mismo como se ve en la siguiente imagen) y conectar los extremos de este a un osciloscopio.

Por ahora no encuentro razones para decantarse por un montaje o el otro más que la practicidad que presenta el primer montaje, tomando en cuenta que requiere menos materiales que el segundo.

Indiferentemente del montaje experimental que se use entre los dos anteriores, la frecuencia de la corriente que pasa por el circuito está dada por la fuente variable en ambos casos, de modo que si se varía la frecuencia generada por la misma con ``pasos'' lo suficientemente pequeños será posible generar una gráfica de amplitud de la corriente versus la frecuencia que permita determinar con cierta precisión la frecuencia de resonancia (aquella con mayor amplitud). En los generadores de funciones el paso más pequeño posible entre una y otra frecuencia se conoce como ``resolución de frecuencia''. En el caso de un generador comercial moderno, por ejemplo el JDS-2900 [3], la resolución de frecuencia es de $0.01\,\mu\text{Hz}$ ($0.00000001\,\text{Hz}$) lo que sería más que suficiente para generar unas gráficas bien precisas que permitan distinguir entre las frecuencias de resonancia que se incluyen en la Tabla 2.

Además se asume que en el montaje experimental se pueda conectar una computadora, o en su defecto un Arduino, que permita controlar tanto los cambios de frecuencia de la fuente variable como la amplitud de la corriente en el circuito simultáneamente y de manera automática, esto con el fin de realizar miles de mediciones en un periodo de tiempo corto y para trazar una gráfica exenta de errores humanos.

\textbf{Estimación de los resultados}
...
Calcular la apreciación de la corriente que se puede medir, es decir los errores en $y$ de las dos gráficas anteriores.

Considerar cómo afectan: temperatura, humedad, presión atmosférica a la frecuencia de resonancia.

Proponer un montaje que minimice sus efectos.

\bigskip

\underline{Referencias:}

[1] Laboratory 5 speed of light\\
\url{https://facultyweb.cortland.edu/douglas.armstead/S15/intermediate/Lab5SpeedOfLight.pdf}

[2] An Electrical Measurement of the Speed of Light. MIT\\
\url{https://web.mit.edu/8.13/www/JLExperiments/JLExp007.pdf}

\medskip
La resolución de un contador de frecuencia comercial es de $0.000006\,\text{Hz}$, por ejemplo este (información en la página 5): \href{https://www.manualslib.com/manual/1606722/Feelelec-Fy6800-Series.html?page=5#manual}{FeelElec FY6800 Series User Manual}.

La resolución de frecuencia de un generador de señal sinusoidal CA es de micro hertz, por ejemplo este tiene resolución de $0.01\,\mu\text{Hz}$ ($0.00000001\,\text{Hz}$).

[3] \url{https://img.banggood.com/file/products/20180129015915JDS2600QuickGuide.pdf}\\
\url{https://articulo.mercadolibre.com.ve/MLV-750552409-generador-senal-contador-precisa-pantalla-lcd-profesional-60-_JM}

[4] Multímetro más preciso: \url{https://www.tek.com/en/datasheet/dmm4050-4040-digital-multimeter}
